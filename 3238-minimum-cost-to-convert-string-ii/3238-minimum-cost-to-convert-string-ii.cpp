...
public:
    long long minimumCost(string source, string target, vector<string>& original, vector<string>& changed, vector<int>& cost) {
      unordered_map<string, int> index;
        ...
        for (int i = 0; i < cost.size(); ++i) dis[index[original[i]]][index[changed[i]]] = min(dis[index[original[i]]][index[changed[i]]], (long)cost[i]);
        ...
        for (int k = 0; k < n; ++k) 
        {
            for (int i = 0; i < n; ++i) 
            {
                if (dis[i][k] < LONG_MAX) 
                {
                    for (int j = 0; j < n; ++j) 
                    {
                        if (dis[k][j] < LONG_MAX) dis[i][j] = min(dis[i][j], dis[i][k] + dis[k][j]);
                    }
                }
            }
        }
        ...
        for (const string& o : original) substrLengths.insert(o.length());
        ...
        for (int i = 0; i < target.length(); ++i) 
        {
            ...
            for (int t : substrLengths) 
            {
                ...
                if (c1 >= 0 && c2 >= 0 && dis[c1][c2] < LONG_MAX) dp[i + t] = min(dp[i + t], dp[i] + dis[c1][c2]);
            }
        }
