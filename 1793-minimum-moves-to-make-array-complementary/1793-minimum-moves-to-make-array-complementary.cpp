...
public:
    ...
    int minMoves(vector<int>& nums, int limit) {
        ...
        for(int i=0;i<nums.size()/2;i++) allsums[nums[i]+nums[n-i-1]]++;
        
        vector<int> omove(2*limit+2),tmove(2*limit+2);  //Maximum sum possible is 2*limit and minimum sum = 2    omove--> One move,  tmove--> two move and these vectors will store the mimum moves required to reach a particular balance value via their indices.
        ...
        // In omove we have to remove sums that have already been acheived i.e in omove we have counted 5 (2+3). Remove that as we can reach '5' naturally
        for(auto i:allsums){            
            omove[i.first]-=i.second;
        }                
        int mm=INT_MAX;
        // min(Sum of omve and 2*tmove) is answer :)
        for(int i=2;i<=2*limit;i++) mm = min(omove[i]+2*tmove[i],mm);
