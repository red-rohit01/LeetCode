...
public:
    ...
    pair<bool,int> dfs(TreeNode* root, int start){
        if(root==nullptr) return {false,0}; //return type pair states whether we came across start or not and maximum distance in this call
        ...
        //If we are still here, it means we have not come across the start node in this dfs call and hence will return maximum of 2 dfs calls
