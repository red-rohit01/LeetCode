...
public:
    vector<int> maximumWeight(vector<vector<int>>& intervals) {
        ...
        // dp[i][k] --> {maximum_weight, list_of_indices} using upto 'k' intervals from the first 'i' available bookings.
        ...
        for (int i=1;i<=n;++i) 
        {
            ...
            for (int k=1;k<=4;++k) 
            {
                ...
                if (take_w > skip.first)  dp[i][k] = {take_w, take_idx};
                else if (take_w == skip.first) 
                {
                    if(take_idx<skip.second) dp[i][k]={take_w, take_idx};
                    else dp[i][k]=skip;
                } 
                else dp[i][k]=skip;
            }
