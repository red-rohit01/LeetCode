...
public:
    ...
    vector<int> countMentions(int numberOfUsers, vector<vector<string>>& events) {
        auto comp = [](vector<string>& a, vector<string>& b){
            if(stoi(a[1]) == stoi(b[1])) return a[0][0] > b[0][0];         // If timestamps are same then M<O
            return stoi(a[1]) < stoi(b[1]);
        };

        vector<int>ans(numberOfUsers), userStatus(numberOfUsers,1);
        priority_queue<pair<int, int>, vector<pair<int, int>>, greater<pair<int,int>>> pq;   //{time_till_offline, userId}
        int all=0;
