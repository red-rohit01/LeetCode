...
class Solution {
    ...
    // No. of arrangements of lenght '5' with [1,2,6] is same as with [2,4,12] or [1,3,6]
    int UniqueArrangements (int length, int mx) {
        ...
        ans=(UniqueArrangements(length-1, mx)%mod + UniqueArrangements(length-1, mx-1)%mod)%mod;
        return ans;
    }
    ...
    int idealArrays(int n, int maxValue) {
        ...
        {            
            for (int j=1;j<21;++j) 
            {
                ...
                ans=(ans%mod +(arrangementsWithEverything * lengths[last][j])%mod)%mod;
            }
        }
        return ans;
    }
